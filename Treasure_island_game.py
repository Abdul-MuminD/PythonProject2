print("""
*******************************************************************************
          |                   |                  |                     |
 _________|________________.=""_;=.______________|_____________________|_______
|                   |  ,-"_,=""     `"=.|                  |
|___________________|__"=._o`"-._        `"=.______________|___________________
          |                `"=._o`"=._      _`"=._                     |
 _________|_____________________:=._o "=._."_.-="'"=.__________________|_______
|                   |    __.--" , ; `"=._o." ,-"'"-._ ".   |
|___________________|_._"  ,. .` ` `` ,  `"-._"-._   ". '__|___________________
          |           |o`"=._` , "` `; .". ,  "-._"-._; ;              |
 _________|___________| ;`-.o`"=._; ." ` '`."\` . "-._ /_______________|_______
|                   | |o;    `"-.o`"=._``  '` " ,__.--o;   |
|___________________|_| ;     (#) `-.o `"=.`_.--"_o.-; ;___|___________________
____/______/______/___|o;._    "      `".o|o_.--"    ;o;____/______/______/____
/______/______/______/_"=._o--._        ; | ;        ; ;/______/______/______/_
____/______/______/______/__"=._o--._   ;o|o;     _._;o;____/______/______/____
/______/______/______/______/____"=._o._; | ;_.--"o.--"_/______/______/______/_
____/______/______/______/______/_____"=.o|o_.--""___/______/______/______/____
/______/______/______/______/______/______/______/______/______/______/[TomekK]
*******************************************************************************
""")

print("Welcome to Treasure Island.\nYour mission is to find the treasure.")
first_choice = input("You are at a cross road. Where do you want to go?\n    type 'left' or right \n").lower()
if first_choice == "right":
    print("You've come to a lake. There is an island in the middle of the lake.")
    second_choice = input("  Type 'wait' to wait for a boat. Type 'swim' to swim across. \n").lower()
    if second_choice == "wait":
        print("You arrive at the island unharmed. There is a house with 3 doors.")
        third_choice = input("   One red, One yellow and One blue. Which color do you choose?\n").lower()
        if third_choice == "yellow":
            print("Congratulation.. You've won!🎉")
        elif third_choice == "red" or third_choice == "blue":
            print("Game over! You chose the wrong door. ")
        else:
            print("Wrong input at third choice")
    elif second_choice == "swim":
        print("Game Over! You should've been patient, you got eaten by a crocodile.")
    else:
        print("Wrong input at second choice!")

elif first_choice == "left":
    print("Game Over! You fell in a hole.")
else:
    print("Wrong input! Try again.")